...
url = 'https://www.avvo.com/expungement-lawyer/al.html'
...
def scrape_attorneys_from_page(soup):
    for attorney in soup.find_all('div', class_='header'):
        ...
        if profile_link:
            ...
            # Extract additional details from the attorney's profile page
            #email = profile_soup.find('a', class_='email-link').text if profile_soup.find('a', class_='email-link') else 'N/A'
            #state_bars = profile_soup.find('div', class_='bar-status').text.strip() if profile_soup.find('div', class_='bar-status') else 'N/A'
            #legal_areas = [area.text for area in profile_soup.find_all('a', class_='area-of-practice')]  # Collecting all areas of expertise
            #legal_areas = ', '.join(legal_areas) if legal_areas else 'N/A'
...
df.to_csv('attorneys_data.csv', index=False)

print('Data scraping complete. Results saved to attorneys_data.csv.')